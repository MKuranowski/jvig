...
async function handleStops (stopId: string, map: L.Map, div: HTMLDivElement): Promise<void> {
  ...
  if (stopData === null) {
    ...
    if ((await ipcRenderer.invoke('exists', 'stops') as boolean)) {
...
async function handleStopTimes (stopId: string, div: HTMLDivElement): Promise<void> {
  ...
  for (const [serviceId, rows] of rowsToView) {
    ...
    if (!wroteHeader) {
      ...
      wroteHeader = true
      const headerElements = Object.keys(rows[0])
    ...
    // Sort rows by time
    rows.sort((a, b) => {
      const aTimeStr = a.departure_time || a.arrival_time // eslint-disable-line @typescript-eslint/strict-boolean-expressions
      const bTimeStr = b.departure_time || b.arrival_time // eslint-disable-line @typescript-eslint/strict-boolean-expressions
