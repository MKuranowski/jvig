...
async function dumpCalendar (table: HTMLTableElement): Promise<void> {
  ...
  ipcRenderer.on('dump-stream-calendar', async (event: IpcRendererEvent, row: Gtfs.Row) => {
    ...
    const elements = Object.entries(row)
      .map(([key, value]) => prepareCalendarValue(key, value))
...
async function dumpDates (table: HTMLTableElement): Promise<void> {
  let wroteHeader: boolean = false
  ...
  ipcRenderer.on('dump-stream-calendarDates', async (event: IpcRendererEvent, rows: Gtfs.Row[]) => {
    ...
    for (const row of rows) {
      ...
      const elements = Object.entries(row)
        .map(([key, value]) => prepareDatesValue(key, value))
