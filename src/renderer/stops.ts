...
async function createMarker (markerGroup: L.FeatureGroup, row: Gtfs.Row): Promise<void> {
...
export async function init (): Promise<void> {
  ...
  const [markerGroup, exists] = await Promise.all([
    createMap(), (ipcRenderer.invoke('exists', 'stops') as Promise<boolean>)
  ])
  ...
  ipcRenderer.on('dump-stream-stops', async (event: IpcRendererEvent, row: Gtfs.Row) => {
    ...
    await Promise.all([
      ...
      (async () => {
        ...
        const elements = [['_link_departures', row.stop_id], ...Object.entries(row)]
